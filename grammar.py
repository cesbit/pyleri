    ...
    def __setitem__(self, key, value):
        ...
        if isinstance(value, NamedElement):
            if hasattr(value, 'name'):
                raise SyntaxError('Element name is set to {0!r} and therefore cannot be set to {1!r}. Use Repeat({0}, 1, 1) as a workaround.'.format(value.name, key))
            value.name = key
...
'''.lstrip()
    ...
    def __init__(self):
        ...
        Note: usually you should only initialize a Grammar instance once in a project.
        '''
    ...
    def export_js(self, js_module_name=JS_MODULE_NAME, js_template=JS_TEMPLATE, js_identation=JS_IDENTATION):
        '''Export the grammar as a JavaScript file which can be used with the js-lrparsing module.'''
        ...
        return js_template.format(
            ...
            language='\n'.join(language),
            arguments=',\n'.join(map(lambda s: js_identation * 3 + s, classes)),
            re_keywords=self.RE_KEYWORDS.pattern,
            constructors=',\n'.join(map(lambda s: js_identation + s, ['.'.join(['window', js_module_name, n]) for n in classes])))

    def parse(self, string):
        '''Parse some string to the Grammar.

        Returns a nodeResult with the following attributes:
         - is_valid: True when the string is successfully parsed by the Grammar.
         - pos: position in the string where parsing ended. (this is the end of the string
                when is_valid is True)
         - expecting: a list containing possible elements at position 'pos' in the string.
         - tree: the parse_tree containing a structured result for the given string.
        '''
        self._string = string
        ...
        node_res = self._walk(self._element, 0, tree.children, self._element, True)
